...
##############################################################################################################################################
	...
	def make_material_request_and_po(self):
		...
		for product, supplier in default_supplier.items():
			grouped_dict[supplier].append(product)

		result_list = [{'supplier': supplier, 'products': products} for supplier, products in grouped_dict.items()]

		for i in result_list:
			mr = frappe.get_doc("Material Request",material_request_list[0].name)
			po = frappe.new_doc("Purchase Order")
			po.company = self.company
			po.supplier = i["supplier"]
			po.schedule_date = self.posting_date
			po.custom_production_plan = self.name

			for row in i["products"]:
				wh = None
				pp_sub_item = None
				qty = 0
				mr_item_name = None
				for j in self.mr_items:
					if j.item_code != row:
						continue
					wh = j.warehouse
					pp_sub_item = j.name
				...
				for mr_item in mr.items:
					if mr_item.item_code != row:
						continue
					
					mr_item_name = mr_item.name


				po.append("items",{
					"item_code": row,
					"custom_production_plan": self.name,
					"material_request": material_request_list[0].name,
					"material_request_item": mr_item_name,
					"warehouse": wh,
					"production_plan_sub_assembly_item": pp_sub_item,
		...
		if material_request_list and purchase_orders:
			material_request_list = [
			...
			purchase_order_list = [
				"""<a href="/app/Form/Purchase Order/{0}">{1}</a>""".format(m, m)
				for m in purchase_orders
			]
...
##############################################################################################################################################
	...
	def get_sub_assembly_items(self, manufacturing_type=None):
		...
		self.set_default_supplier_for_all_sub_assembly()
	...
	def set_default_supplier_for_all_sub_assembly(self):
		items = [
			d.production_item for d in self.sub_assembly_items
		]
		...
		default_supplier = frappe._dict(
			frappe.get_all(
				"Default Supplier List",
				fields=["parent", "default_supplier_name"],
				filters={"parent": ("in", items), "slot_pengiriman": self.slot_pengiriman, "day": "Weekdays"},
				as_list=1,
			)
		)
